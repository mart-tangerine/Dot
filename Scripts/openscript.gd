...
func _ready() -> void:
	...
	while true:
		...
		if get_parent().name != "PHolders":
			if root.get_node("FE").data.has(str(get_parent().name)):
				if root.get_node("FE").data[str(get_parent().name)].has(str(name)):
					if root.get_node("Files/Files/Hernya/Fles").has_node(root.get_node("FE").data[str(get_parent().name)][str(name)]["Sprite"]):
						$Panel/Icon.texture = root.get_node("Files/Files/Hernya/Fles").get_node(root.get_node("FE").data[str(get_parent().name)][str(name)]["Sprite"]).get_node("Panel/Icon").texture


func _on_button_pressed() -> void:
	get_parent().get_parent().get_parent().get_parent().get_node("Click").play()
	get_parent().get_parent().get_parent().get_parent().get_node("Scripts").show()
	for x in get_parent().get_parent().get_parent().get_parent().get_node("Scripts").get_children():
		x.hide()
	get_parent().get_parent().get_parent().get_parent().get_node("Scripts/" + str(get_parent().name)).show()
	for x in get_parent().get_parent().get_parent().get_parent().get_node("Scripts/" + str(get_parent().name)).get_children():
		if x.name == name:
			x.show()
		else:
			x.hide()
